...
ggplot(Boston, aes(x = lstat, y = medv)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), color = "pink")
...
-   Basically what I found out online was that poly(lstat, 5) breaks down the data into 5 different polynomials. It says that the first estimate is the linear component while the following 4 different outputs are representing the refined curve by pulling data from various different places to fit the line better.
...
```{r}
lm.simple1 <- lm(medv ~ lstat * age, data = Boston) 
```

```{r}
simplescatter <- ggplot(Boston, aes(x = medv, y = lstat * age)) +
  geom_point()
...
```{r}
lm.selena <- lm(medv ~ lstat * age + I(lstat^2), data = Boston) 
summary(lm.selena)
```

```{r}
Selena <- ggplot(Boston, aes(x = lstat + age, y = medv)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x + I(x^2), color = "pink")
